...
class RobotNavigator(Node):
    ...
    # Utilizar o sensor laser
    def sector_min(self, idx_start: int, idx_end: int) -> float:
        if not self.ranges:
            return float('inf')

        sector = self.ranges[idx_start:idx_end]
        valid_values = [r for r in sector
    ...
    def get_sectors(self):
        ...
        if n == 0:
            inf = float('inf')
            return {'front': inf, 'front_left': inf, 'front_right': inf, 'left': inf, 'right': inf,}
        ...
        return {
            ...
            'front_left': self.sector_min(center, center + f50),
            'front_right': self.sector_min(center - f50, center),
    ...
    def dist_to_m_line(self, target_x, target_y):
        ...
        return abs(dy * self.x - dx * self.y + target_x * sy - target_y * sx) / length
    ...
    def _wall_follow(self, obs):
        ...
        if self.follow_side == 'right':
            ...
            side_dist = obs['left']
            forward_blocked = (
                obs['front'] < OBSTACLE_THRESHOLD
                or obs['front_left'] < OBSTACLE_THRESHOLD * 0.8
            )
